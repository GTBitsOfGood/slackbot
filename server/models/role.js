...
Role.hasPermission = function hasPermission(role, permissionLevel) {
  ...
  if (roleIndex === -1) {
    throw new TypeError("the role must be one of the values of the Role enum");
  }
  if (permissionLevelIndex === -1) {
    throw new TypeError("the permissionLevel must be one of the values of the Role enum");
  }
